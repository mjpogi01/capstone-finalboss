-- =====================================================
-- Fix Order Status Categorization in Analytics
-- =====================================================
-- This updates the RPC function to properly categorize
-- order statuses for completed and processing orders
-- =====================================================
...
BEGIN
  ...
  -- FIXED: Properly categorize order statuses
  EXECUTE format('
      ...
      ''orderStatus'', (
        ...
        categorized AS (
          SELECT 
            ...
            SUM(CASE WHEN status_lower IN (''confirmed'', ''layout'', ''sizing'', ''printing'', ''press'', ''prod'', ''packing_completing'', ''processing'', ''in_production'', ''ready_for_pickup'', ''ready_for_delivery'') THEN count ELSE 0 END) AS processing_count,
            SUM(CASE WHEN status_lower IN (''pending'', ''payment_pending'', ''awaiting_payment'', ''awaiting_confirmation'') THEN count ELSE 0 END) AS pending_count,
